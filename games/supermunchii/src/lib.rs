//! Super Munchii — the sample side-scrolling platformer that exercises the scamper
//! engine. Exposed as a library so the arcade launcher can start it via [`launch`];
//! the thin `main.rs` binary just forwards its argv to [`run_cli`] for the standalone
//! `scamp <subcommand>` workflow (live play, headless `verify`, replay, etc.).
...
/// (argv, including argv[0]). Split out of `main` so the binary is a thin forwarder
/// and the launcher can reuse the crate as a library.
        ...
        if help {
            ...
            if now < intro_until {
                ...
                if let Some(&b) = bests.get(&level.id) {
                    let (mg, ms) = level.medals.unwrap_or((25, 55));
                    best_line = format!(" best {}:{:02}  {} ", b / 60, b % 60, medal_for(b, mg, ms).0);
            ...
            if won && !game_over {
                let (mg, ms) = level.medals.unwrap_or((25, 55));
                let (medal, stars) = medal_for(secs, mg, ms);
